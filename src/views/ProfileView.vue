...
const currentPasswordError = computed(() => {
  ...
  if (!verifyPassword(passwordForm.current)) return 'Current password is incorrect.'
  return ''
})
...
const isPasswordFormValid = computed(() => (
  ...
  && verifyPassword(passwordForm.current)
...
function savePassword() {
  ...
  if (!isPasswordFormValid.value || !changePassword(passwordForm.current, passwordForm.next)) return
  ...
  passwordStatus.value = { variant: 'success', title: 'Password updated', message: 'Use your new password the next time you sign in.' }
}
      ...
      <div class="profile-layout">
        ...
        <form class="profile-form personal-form" @submit.prevent="saveProfile">
          ...
          <div class="profile-form__grid">
            ...
            <label class="full"><span>Email</span><input v-model.trim="profile.email" type="email" required /></label>
            <label class="full"><span>Phone number</span><input v-model.trim="profile.phone" type="tel" /></label>
          </div>
          <div class="profile-form__footer"><p v-if="saved"><Check :size="16" />Profile saved</p><button type="submit">Save changes</button></div>
        </form>
      </div>
    </main>
    <AppFooter />
    ...
    <div v-if="securityOpen" class="security-backdrop" @click.self="closeSecurity">
      <form ref="securityDialog" class="security-dialog" role="dialog" aria-modal="true" aria-labelledby="security-title" tabindex="-1" @submit.prevent="savePassword">
        ...
        <div class="security-fields">
          <label><span>Current password</span><div :class="{ invalid: currentPasswordError }"><LockKeyhole :size="17" /><input v-model="passwordForm.current" :type="showPasswords ? 'text' : 'password'" autocomplete="current-password" :aria-invalid="Boolean(currentPasswordError)" aria-describedby="current-password-error" @blur="passwordTouched.current = true" /></div><small v-if="currentPasswordError" id="current-password-error" class="security-error">{{ currentPasswordError }}</small></label>
        ...
        <div class="security-dialog__actions"><button type="button" @click="closeSecurity">Cancel</button><button class="primary" type="submit" :disabled="!isPasswordFormValid">Update password</button></div>
      </form>
    </div>
  </div>
</template>
